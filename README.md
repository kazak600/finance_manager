...
Реєстрація через API закрита для безпеки. Щоб додати нового користувача, скористайтеся скриптом для генерації SQL-запиту:

1. Згенеруйте SQL-запит:
```bash
node scripts/add-user.js email@example.com "ваш-пароль"
```

2. Виконайте отриману команду `npx wrangler d1 execute...` для локальної або віддаленої бази.
