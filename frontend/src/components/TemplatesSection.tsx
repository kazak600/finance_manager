    ...
    <section className="mt-6 rounded-xl border border-slate-200 bg-white p-4">
      ...
      <ul className="space-y-2">
        {templates.map((template) => (
          <li key={template.id} className="flex flex-wrap items-center justify-between gap-2 rounded border border-slate-200 p-3">
            <div>
            ...
            <div className="flex gap-2">
              ...
              <button className="rounded bg-slate-200 px-2 py-1 text-xs" onClick={() => onEditTemplate(template)}>
                Edit
              </button>
        ...
        {templates.length === 0 && <li className="text-sm text-slate-500">Шаблони ще не створено</li>}
      </ul>
    </section>
  )
}
