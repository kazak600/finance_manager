...
export function MonthlyStatsCard({ stats, fallbackMonth, formatMoney }: Props) {
  return (
    <article className="rounded-xl border border-slate-200 bg-white p-4 md:col-span-2">
      <p className="mb-2 text-xs uppercase text-slate-500">Місячна статистика ({stats?.month ?? fallbackMonth})</p>
      ...
      <ul className="grid gap-2 sm:grid-cols-2">
        {(stats?.categories ?? []).map((item) => (
          <li key={`${item.type}-${item.category}`} className="rounded bg-slate-50 px-3 py-2 text-sm">
